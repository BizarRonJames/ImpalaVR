...
namespace Autohand{
    ...
    public class Teleporter : MonoBehaviour{
        ...
        private volatile bool inTransit;
        private Vector3 targetPosition;
        private void Start() {
            inTransit = false;  
            playerBody = FindObjectOfType<AutoHandPlayer>();
            print(playerBody.ToString());
        ...
        void Update(){
            if(!inTransit){
                if(aiming){
                    ...
                    var go = LocationNodeManager.Instance.gameObject;
                    foreach (Transform trans in go.GetComponentsInChildren<Transform>(true)){
                        trans.gameObject.layer = 15;
                    }
                }
                else{
                    LocationNodeManager.Instance.closeActive();

                    var go = LocationNodeManager.Instance.gameObject;
                    foreach (Transform trans in go.GetComponentsInChildren<Transform>(true)){
                        trans.gameObject.layer = 25;
                    }
        ...
        void CalculateTeleport() {
            ...
            for(i = 0; i < lineSegments; i++) {
                ...
                if(i != 0) {
                    if(Physics.Raycast(lineArr[i-1], lineArr[i]-lineArr[i-1], out aimHit, Vector3.Distance(lineArr[i], lineArr[i-1]), (~Hand.GetHandsLayerMask()), QueryTriggerInteraction.Collide)) {
                        ...
                        if(Vector3.Angle(aimHit.normal, Vector3.up) <= maxSurfaceAngle && layer == (layer | (1 << aimHit.collider.gameObject.layer))) {
                            ...
                            // check if we hit a location node and open it
                            if( aimHit.transform.gameObject.CompareTag("LocationNode"))
                            {   


                               // print("LOG: hit location node");

                                line.colorGradient = canTeleportColor;
                                lineList.Add(aimHit.point);
                                hitting = true;

                                LocationNodeManager.Instance.updateActive(
                        ...
                        if(aimHit.collider.gameObject.tag != "Hazard") break;
         ...
         *  makes use of a couroutine lock, to only allow for one teleport at a time
         * </summary>
        **/
        private IEnumerator fadeAnimation(){
                    ...
                    Vector3 toPos = new Vector3(aimHit.point.x,aimHit.point.y,aimHit.point.z);
                    playerBody.SetPosition(toPos);
                    yield return new WaitForSeconds(0.1f);
